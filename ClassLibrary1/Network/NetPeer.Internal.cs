...
namespace Libgren.Network
{
	public partial class NetPeer
	{
		...
		private void Heartbeat()
		{
			...
			if (m_socket == null)
				return;
            if (m_socket.SendFirst)
            {
                m_socket.StartReceive(m_receiveBuffer, 0, m_receiveBuffer.Length, ref m_senderRemote);
            }
			if (!m_socket.Poll(1)) // wait up to 1 ms for data to arrive
				return;
			...
			do
			{
				...
				try
				{
                   // bytesReceived = m_socket.ReceiveFrom(m_receiveBuffer, 0, m_receiveBuffer.Length, ref m_senderRemote);
                    lock (m_socket.recLock)
                    {
                        bytesReceived = m_socket.BufferTransferred.Dequeue(); ;
                        m_receiveBuffer = m_socket.BufferQueue.Dequeue();
                        m_senderRemote = m_socket.BufferSender.Dequeue();
                    }
				}
                ...
                //m_senderRemote = new IPEndPoint(IPAddress.Parse("127.0.0.1"), 10000);
		...
		private void ReceivedUnconnectedLibraryMessage(double now, IPEndPoint senderEndpoint, NetMessageType tp, int ptr, int payloadByteLength)
		{
            ...
            Console.WriteLine("Handshake added!");
